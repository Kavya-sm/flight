...
export default {
  ...
  async beforeMount() {
    try {
      // Only fetch if we don't already have the flight data
      if (this.isAuthenticated && !this.selectedFlight) {
  ...
  data() {
    return {
      ...
      // Initialize with the flight prop directly - this prevents duplication
      selectedFlight: null
    };
  },
  watch: {
    // Watch for flight prop changes and update selectedFlight
    flight: {
      immediate: true,
      handler(newFlight) {
        if (newFlight) {
